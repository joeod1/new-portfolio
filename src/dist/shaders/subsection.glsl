...
void main() {
    lowp float factor = 100.;//5.;
    // lowp float test = snoise(gl_FragCoord.xy / 1000.0 - u_time * 0.01);
  lowp float test = sin((u_time / 10. + (gl_FragCoord.x + gl_FragCoord.y / factor)/40.)) + 0.5;

  test *= cos(u_time / 6. + (gl_FragCoord.x  / factor + gl_FragCoord.y)/ 40.) + 0.5;
//   test *= sqrt(
//     (u_mouse.x - gl_FragCoord.x) * (u_mouse.x - gl_FragCoord.x)
//     + (u_mouse.y - gl_FragCoord.y) * (u_mouse.y - gl_FragCoord.y)
//     ) / 10.;
    test = step(gl_FragCoord.x + sin(gl_FragCoord.y / 10. + u_time * 2.) * 10., 50.);
    test = clamp(test, 0., 1.);
    // test = mod(test, 1.6);
    // test *= sqrt(
    // (u_mouse.x - gl_FragCoord.x) * (u_mouse.x - gl_FragCoord.x)
  ...
  gl_FragColor = vec4(test - 0.7, test - 0.7, test - 0.7, 0.1);
}